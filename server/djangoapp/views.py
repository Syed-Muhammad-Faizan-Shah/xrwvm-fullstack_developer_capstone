...
def get_cars(request):
    ...
    # Run populate only if models table is empty
...
def logout_request(request):
    if request.user.is_authenticated:
        ...
        return JsonResponse({"status": False, "error": "User not logged in"})
# ...
...
def get_dealer_reviews(request, dealer_id):
    if dealer_id:
        ...
        for review_detail in reviews:
            ...
            # ensure 'review' key exists before sentiment analysis
            text = review_detail.get("review", "")

            try:
                sentiment = analyze_review_sentiments(text).get("sentiment", "neutral")
            except:
                sentiment = "neutral"
