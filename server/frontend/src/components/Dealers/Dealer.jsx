...
const Dealer = () => {
  ...
  const get_reviews = async () => {
    ...
    console.log("Raw reviews response:", retobj);   // 🔥 ADD THIS
  
    if (retobj.status === 200) {
      if (retobj.reviews.length > 0) {
        console.log("Fetched reviews:", retobj.reviews);  // 🔥 ADD THIS
        setReviews(retobj.reviews);
      } else {
        console.log("No reviews found at backend");       // 🔥 ADD THIS
        setUnreviewed(true);
      }
    }
  }
      ...
      <div className="reviews_panel">
        {reviews.length === 0 && !unreviewed ? (
          ...
          reviews.map((review, index) => {
            console.log("Sentiment:", review.sentiment);   // ⭐ ADDED HERE
